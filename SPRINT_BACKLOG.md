...
* [Task 1.1] Configurar Estrutura de Camadas (DDD):
       * Ação: Esta tarefa continua válida e segura, pois é puramente aditiva. Criar os diretórios backend/domain e
         backend/infrastructure não afeta o código existente.
       * Status: Já foi concluída corretamente na minha ação anterior.

   * [x] [Task 1.2] Implementar Conexão com Neo4j e Redis:
       * Ação Corrigida:
           1. NÃO TOCAR no backend/database.py. Ele permanecerá como o ponto de acesso para o PostgreSQL.
           2. Criar um NOVO arquivo: backend/core/connections.py. Este arquivo será o lar exclusivo para a configuração
              e as funções de dependência (get_neo4j_session, get_redis_client). Isso isola as novas conexões.
           3. Verificar se as variáveis REDIS_HOST e REDIS_PORT já estão sendo validadas no backend/core/env.py. Se não
              estiverem, adicioná-las de forma segura à função validate_environment, sem remover as existentes.
       * Resultado: O sistema terá dois gerenciadores de conexão distintos: database.py para o SQL e core/connections.py
         para o NoSQL (Grafo/Cache).

   * [x] [Task 1.3] Criar Endpoint de Health Check:
       * Ação Corrigida:
           1. Criar backend/api/health_check.py conforme planejado.
           2. O endpoint GET /health irá injetar AMBAS as dependências: get_db (de database.py) e get_neo4j_session (do
              novo core/connections.py).
           3. O endpoint retornará o status de todos os serviços: {"status": "ok", "postgresql": "connected", "neo4j":
              "connected", "redis": "connected"}.
       * Resultado: Teremos um endpoint de saúde completo que monitora toda a infraestrutura de dados da aplicação.
...
- [x] **[Task 2.1] Endpoint de Submissão de Tentativa**
...
- [x] **[Task 2.2] Serviço de Domínio (Use Case)**
...
- [x] **[Task 2.3] Repositório de Diagnóstico (Infra)**
...
- [x] **[Task 2.4] Expor Endpoint do Diagnóstico de Raiz**
...
- [x] **[Task 3.1] Setup do SvelteKit 5 e Estrutura de Rotas**
...
- [x] **[Task 3.2] Componente Atômico: `ConceptNode.svelte`**
...
- [x] **[Task 3.3] Componente Atômico: `EdgeLayer.svelte`**
...
- [x] **[Task 4.1] Carregamento de Dados (SSR)**
...
- [x] **[Task 4.2] Componente de Orquestração: `GraphViewport.svelte`**
...
- [x] **[Task 4.3] Implementar Interatividade de Seleção**
...
- [x] **[Task 4.4] Conectar Diagnóstico de Raiz (Client-Side)**
